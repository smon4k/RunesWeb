...
export function keepDecimalNotRounding(value, count, isZero) {
  ...
  if(value !== undefined) {
    ...
    // 加0是为了第二种情况
    number = integer + '.' + (decimal + '0'.repeat(countNum - 1)).substr(0, countNum);
    if (isZero) {
...
//交易Hash字符串截取处理
export const topicsHashString = (str = '', length = 5, number = 1, resString = true) => {
  let strNumber = str.substring(str.length - length);
  if (resString) {
    let num = strNumber.replace(/[^0-9]/ig, "");
    if (num !== '' && Number(num) >= 0) {
      let result = num.substring(num.length - number);
