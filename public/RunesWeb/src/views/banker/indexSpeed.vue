    ...
    <div class="container">
        ...
        <el-row class="dealer-box" :style="{width: isMobel ? '100%' : '90%'}"> 
            <el-col :span="isMobel ? 24 : 24" :align="isMobel ? 'center' : 'center'">
                ...
                <el-col :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                    <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                        ...
                        <span class="amount">{{ keepDecimalNotRounding(totalBalance, decimalLen, true) }} {{ transactionSpareCurrency }}</span>
                    </div>
                </el-col>
                <!-- 总盈利额 -->
                <el-col :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                    <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                        ...
                        <span class="amount">{{ calcAllProfit() }} {{ transactionSpareCurrency }}</span>
                    </div>
                </el-col>
                <!-- 总收益率 -->
                <el-col v-show="!isMobel" :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                    <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                        <span class="name">{{ $t('game:SiteProfitRatio') }}：</span>
                        <span class="amount">{{ calcSiteProfitRatio() }}%</span>
                    </div>
                </el-col>
                <!-- 我的总余额 -->
                <el-col :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                    <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                        <span class="name">{{ $t('game:myInvested') }}：</span>
                        <span class="amount">{{ keepDecimalNotRounding(walletBalance, decimalLen, true) }} {{ transactionSpareCurrency }}</span>
                    </div>
                </el-col>
                <!-- 我的占比 -->
                <el-col :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                ...
                <el-col :span="isMobel ? 12 : 8" :align="isMobel ? 'center' : 'center'">
                    <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                        ...
                        <span class="amount">{{ calcProfit() }} {{ transactionSpareCurrency }}</span> 
                    </div>
                </el-col>
            </el-col>
            ...
            <!-- <el-col :span="isMobel ? 8 : 6" :align="isMobel ? 'center' : 'left'">
                <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                    <span>庄家总余额：{{ toFixed(totalBalance, 4) }} {{ transactionSpareCurrency }}</span><br>
                    <span>我的余额：{{ toFixed(walletBalance, 4) }} {{ transactionSpareCurrency }}</span><br>
                </div>
            </el-col>
            <el-col :span="isMobel ? 8 : 6" :align="isMobel ? 'center' : 'left'">
                <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                    <span>总盈利额：{{ calcAllProfit() }} {{ transactionSpareCurrency }}</span><br>
            ...
            <el-col :span="isMobel ? 8 : 6" :align="isMobel ? 'center' : 'left'">
                <div class="proportion" :style="{marginBottom: isMobel ? '10px' : ''}">
                    <span></span><br>
                    <span>盈利额：{{ calcProfit() }} {{ transactionSpareCurrency }}</span> 
                </div>
            </el-col>
        ...
        <el-tabs class="list-tabs" v-model="listTabsActive" @tab-click="handleClicks" :stretch="true"  :style="{width: isMobel ? '100%' : '100%', marginTop: isMobel ? '' : ''}">
            <!-- 庄家汇总 -->
            <el-tab-pane :label="$t('game:bookmakerSummary')" name="1">
                <el-table :data="userDealerProportion" style="width: 100%;" v-loading="userDealerProportion.loading" height="500">
                    ...
                    <!-- 庄家总余额 -->
                    <el-table-column prop="" :label="$t('game:dealerBalance') + '('+transactionSpareCurrency+')'" align="center">
                        <template slot-scope="scope">
                            <span>{{ keepDecimalNotRounding(Number(scope.row.user_balance), decimalLen, true)}}</span>
                        </template>
                    </el-table-column>
                    <!-- 庄家占比 -->
                    <el-table-column prop="" :label="$t('game:proportionOfDealers')" align="center">
                        <template slot-scope="scope">
                            <span>{{ keepDecimalNotRounding(Number(scope.row.proportion), 4)}} %</span>
                        </template>
                    </el-table-column>
                    <!-- 盈利额 -->
                    <el-table-column prop="" :label="$t('game:profit') + '('+transactionSpareCurrency+')'" align="center">
                        <template slot-scope="scope">
                            <span>{{ keepDecimalNotRounding(Number(scope.row.profit), decimalLen)}}</span>
                        </template>
                    </el-table-column>
                    <!-- 收益率 -->
                    <el-table-column prop="" :label="$t('game:rate')" align="center">
                        <template slot-scope="scope">
                            <span>{{ keepDecimalNotRounding(Number(scope.row.rate * 100), 2)}}%</span>
                        </template>
                    </el-table-column>
                </el-table>
            </el-tab-pane>
            <!-- 所有坐庄记录 -->
            <el-tab-pane :label="$t('game:allRecords')" name="2">
                <el-table :data="tableData" style="width: 100%;" v-loading="loading" height="500">
                    ...
                    <el-table-column prop="local_balance" :label="$t('game:Amount') + '('+transactionSpareCurrency+')'" align="center">
                        <template slot-scope="scope">
                        <span>{{ keepDecimalNotRounding(Number(scope.row.amount), decimalLen, true)}}</span>
                        </template>
                    </el-table-column>
            ...
            <!-- 我的庄家汇总 -->
            <el-tab-pane :label="$t('game:mySeatRecord')" name="3">
                ...
                <el-row class="balance">
                    <el-col :span="24">
                        <!-- <div>
                            ...
                            <span @click="WalletBalanceAll()" style="color:#409EFF;text-decoration: underline;cursor:pointer;">{{ toFixed(Number(transactionCurrencyBalance), 4) }} {{ transactionSpareCurrency }}</span>
                        </div> -->
                        <div>
                            {{ $t('game:platformBalance') }}：
                            <span @click="PlatformBalanceAll()" style="color:#409EFF;text-decoration: underline;cursor:pointer;">{{ keepDecimalNotRounding(Number(platformBalance), this.decimalLen, true) }} {{ transactionSpareCurrency }}</span>
                        </div>
                        <!-- <div>{{ $t('game:platformBalance') }}：{{ toFixed(platformBalance, 4) }} {{ transactionSpareCurrency }}</div> -->
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="24">
                        <el-form :model="depositForm" ref="depositForm" :rules="depositFormRules">
                            <el-form-item prop="amount">
                                ...
                                >
                                    <template slot="prepend">{{ transactionSpareCurrency }}</template>
                                </el-input>
                            </el-form-item>
                            ...
                            <div class="submit-name">
                                ...
                                <el-button type="primary" :loading="trading" :disabled="trading || isStatus || isWithdraw" @click="submitForm('depositForm', 1)">{{ $t('game:upper') }}</el-button>
        ...
        <el-dialog
            ...
            <div class="zhuang">
                <el-row class="balance" :style="{padding: isMobel ? '30px' : '0'}">
                    <el-col :span="24">
                        <div>
                            <!-- <div>{{ $t('game:myTotalBalance') }}：{{ toFixed(walletBalance, 4) }} {{ transactionSpareCurrency }}</div> -->
                            <div>
                                {{ $t('game:myInvested') }}：
                                <span @click="TotalBalanceAll()" style="color:#409EFF;text-decoration: underline;cursor:pointer;">{{ keepDecimalNotRounding(Number(walletBalance), this.decimalLen) }} {{ transactionSpareCurrency }}</span>
                            </div>
                            <!-- <div>
                                {{ $t('game:walletBalance') }}：
                                <span>{{ toFixed(Number(transactionCurrencyBalance), 4) }} {{ transactionSpareCurrency }}</span>
                            </div> -->
                            <!-- <br /> -->
                        </div>
                    </el-col>
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form  :model="withdrawForm" ref="withdrawForm" :rules="withdrawFormRules">
                            ...
                            <div class="submit-name">
                                <!-- <el-button type="primary" :loading="trading" :disabled="trading" @click="startApprove" v-if="!approve">{{ $t('swap:Approve') }}</el-button> -->
                                <el-button type="primary" :loading="trading" :disabled="trading || isStatus || isGame || isWithdraw" @click="submitForm('withdrawForm', 2)">{{ $t('game:lower') }}</el-button>
...
import {getBalance,isApproved, getDiceDealerBalance, getDiceBalanceOf, getFillingIncreasingId, getUserDealerBalance, getUserBalanceInfo, getUserPlatformBalance} from "@/wallet/serve";
...
export default {
            ...
            withdrawFormRules: {
                amount: [
                    { required: true, validator: (rule, value, callback) => {
                        ...
                        if(Number(value) > this.walletBalance) { ////余额不足
                            return callback(new Error(this.$t('swap:InsufficientBalance')));
                        } else {
                            callback();
                        }
    ...
    methods: {
        async getAllListData(ServerWhere) { // 所有坐庄记录
        ...
        async getMyListData(ServerWhere) { // 我的坐庄记录
            if (!ServerWhere || ServerWhere == undefined || ServerWhere.length <= 0) {
                ServerWhere = {
        ...
        calcProfit() {  //计算盈利额
            ...
            return keepDecimalNotRounding(profit, this.decimalLen, true);
        },
        ...
        calcSiteProfitRatio() { //总收益率
            let profit = this.calcAllProfit();
            let num = this.totalBalance ? profit / this.totalBalance * 100 : 0;
            return keepDecimalNotRounding(num, 2, true);
        },
        shangZhuangClick() {
            ...
            this.timeBankerInterval = setInterval(async() => {
                ...
                if(this.listTabsActive == 1) {
                    if(this.versionName == 'chain') {
                        ...
                        let dealerBalance = await getUserDealerBalance(this.game_type, 2);
                    ...
                    this.$store.dispatch("refreshUserDealerProportion", {game_type: this.game_type, isLoding: true});
        ...
        async submitForm(formName, type) {//1. 提交调用合约
            ...
            if (valid) {
                ...
                await axios.post(this.apiUrl + '/Api/Dealerrecord/depositWithdraw', params).then(async (json) => {
                    loading.close();
                    this.trading = false;
        ...
        async getBalanceInfo() {
            let dealerBalance = await getUserDealerBalance(this.game_type, 2);
            this.walletBalance = dealerBalance.user_balance;
            this.totalBalance = dealerBalance.total_balance;
        ...
        async TotalBalanceAll() { //点击我的总余额
            // console.log(222);
            const decimals = await this.getCurrentDecimals();
            this.withdrawForm.amount = keepDecimalNotRounding(this.walletBalance, decimals, true);
        },
        async PlatformBalanceAll() { //点击平台余额
            const decimals = await this.getCurrentDecimals();
            this.depositForm.amount = keepDecimalNotRounding(Number(this.platformBalance), decimals, true);
        },
        ...
        async onFunDepositFormInput(value) {
            ...
            if(value > this.platformBalance) {
                value = this.platformBalance.toString();
            }
            const decimals = await this.getCurrentDecimals();
            if(value.indexOf('.') > 0) {
                this.depositForm.amount = value.slice(0,value.indexOf('.')+ (decimals + 1))
            }
        },
        async onWithdrawFunInput(value) {
            ...
            if(value > this.walletBalance) {
                value = this.walletBalance.toString();
            }
            // console.log(value);
            const decimals = await this.getCurrentDecimals();
            if(value.indexOf('.') > 0) {
                this.withdrawForm.amount = value.slice(0,value.indexOf('.')+ (decimals + 1))
            }
        },
    },
};
